...
class CFPaymentGatewayService {
  ...
  setCallback(final void Function(String) vp, final void Function(CFErrorResponse, String) error) {
    if (vp == null || error == null) {
      throw Exception('Callback functions cannot be null');
    }
    ...
    methodChannel.invokeMethod("response").then((value) {
      ...
      try {
        ...
        if (!body.containsKey('status')) {
          throw Exception('Invalid response format: status not found');
        }
        ...
        switch (status) {
          case "exception":
            if (!body.containsKey('data') || !(body['data'] is Map)) {
              throw Exception('Invalid response format: data not found or invalid');
            }
            var data = body["data"] as Map<String, dynamic>;
            var message = data["message"] as String? ?? 'Unknown error';
            onError!(CFErrorResponse(
              'EXCEPTION',
              message,
              'EXCEPTION',
          ...
          case "success":
            ...
            if (!data.containsKey('order_id')) {
              throw Exception('Invalid response format: order_id not found');
            }
          ...
          case "failed":
            ...
            var data = body["data"] as Map<String, dynamic>;
            if (!data.containsKey('order_id') || !data.containsKey('message')) {
              throw Exception('Invalid response format: required fields missing');
            }
            var errorResponse = CFErrorResponse(
              data["status"] as String? ?? 'FAILED',
              data["message"] as String,
              data["code"] as String? ?? 'PAYMENT_FAILED',
              data["type"] as String? ?? 'PAYMENT_ERROR'
            );
      ...
      onError!(CFErrorResponse(
        'ERROR',
        'Error communicating with payment gateway: $e',
        'COMMUNICATION_ERROR',
        'SYSTEM_ERROR'
      ), '');
    });
